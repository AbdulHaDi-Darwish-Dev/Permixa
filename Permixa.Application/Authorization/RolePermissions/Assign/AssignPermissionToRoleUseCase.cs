...
public sealed class AssignPermissionToRoleUseCase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (await _rolePermissionRepository.ExistsAsync(command.RoleId, command.PermissionId, cancellationToken))
            return Result.Failure(AuthorizationErrors.RolePermissionAlreadyExists);

        var state = await GetOrCreateAuthorizationStateAsync(cancellationToken);

        await _rolePermissionRepository.AddAsync(
            RolePermission.Create(command.RoleId, command.PermissionId, _clock.UtcNow),
            cancellationToken);
        ...
        await _audit.WriteAsync(
            IamAuditEvent.Success(
                IamAuditEvents.RolePermissions.Assigned,
                now,
    ...
    private async Task<AuthorizationState> GetOrCreateAuthorizationStateAsync(CancellationToken cancellationToken)
    {
        var state = await _authorizationStateRepository.GetAsync(cancellationToken);
        if (state is not null)
            return state;

        state = AuthorizationState.CreateInitial();
        await _authorizationStateRepository.AddAsync(state, cancellationToken);
        return state;
    }
}
